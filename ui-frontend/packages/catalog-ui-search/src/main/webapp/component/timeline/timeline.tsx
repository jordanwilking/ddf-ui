...
export const Timeline = (props: TimelineProps) => {
  ...
  useEffect(() => {
    ...
    d3.select(d3ContainerRef.current).on('mousemove', function () {
      ...
      const hoverDate = convertDateToTimezoneMoment(
      ...
      return (
        d3
          ...
          .on('start', () => {
            ...
            const newLeftDate = convertDateToTimezoneMoment(
          ...
          .on('drag', () => {
            if (props.mode !== 'single') {
              ...
              const initialDate = convertDateToTimezoneMoment(
                xScale.invert(clickStart),
                props.timezone
              )

              let dragCurrent = clickStart + diff
              const dragDate = convertDateToTimezoneMoment(
    ...
    const getEdgeDrag = (slider: 'LEFT' | 'RIGHT') =>
      d3
        ...
        .on('drag', () => {
          ...
          const dateWithTimezone = convertDateToTimezoneMoment(
            dragValue,
            props.timezone
          )
    ...
    const getBrushDrag = () =>
      d3
        ...
        .on('drag', () => {
          ...
          const newRightDate = convertDateToTimezoneMoment(
